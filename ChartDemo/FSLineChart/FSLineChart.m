...
- (void)setChartData:(NSArray *)chartData
{
    ...
    NSLog(@"self.frame.height %f", CGRectGetHeight(self.frame));
    NSLog(@"_axisHeight %f", _axisHeight);
    NSLog(@"_margin %f", _margin);
    
    if(_labelForValue) {
        for(int i=0;i<_verticalGridStep;i++) {
            CGPoint p = CGPointMake(_margin + (_valueLabelPosition == ValueLabelRight ? _axisWidth : 0), _axisHeight + 2 * _margin - (i + 1) * _axisHeight / _verticalGridStep - 14);
